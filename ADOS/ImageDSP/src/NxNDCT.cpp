...
void IDCT(const short input[], char output[], int N, double* DCTKernel)
{
    ...
    for (int i = 0; i <= N - 1; ++i)
    {
        for (int j = 0; j <= N - 1; ++j)
        {
            sum = 0;
            for (int k = 0; k <= N - 1; ++k)
                sum += DCTKernel[k*N+i] * (input[k*N+j]);
    ...
    for (int i = 0; i <= N - 1; ++i)
    {
        for (int j = 0; j <= N - 1; ++j)
        {
            sum = 0;
            for (int k = 0; k <= N - 1; ++k)
                sum += temp[i*N+k] * DCTKernel[k*N+j];
    ...
    for (int i = 0; i < N*N; ++i)
    {
        int val = (int)floor(DCTCoefficients[i] + 0.5) + 128;
