        ...
        for(int xx = 0; xx < x; xx += 2){
            ...
            for(int yb = 0; yb < 2; yb++){
                for(int xb = 0; xb < 2; xb++){
                    ...
                    Y_buff[(yy+yb)*x + (xx+xb)] =  0.299*R + 0.587*G + 0.114*B;
                    U += (- 0.14713*R - 0.28886*G + 0.436*B);
                    V += (0.615*R - 0.51499*G - 0.10001*B);
                }
            }
            U_buff[(yy/2)*x/2 + xx/2] =  U/4;
            V_buff[(yy/2)*x/2 + xx/2] =  V/4;
        }
    }
}
...
void procesing_YUV420(uchar Y_buff[], char U_buff[], char V_buff[], int x, int y, double Y, double U, double V)
{
    ...
    for(int yy = 0; yy < y; yy += 2){
        for(int xx = 0; xx < x; xx += 2){
            for(int yb = 0; yb < 2; yb++){
                for(int xb = 0; xb < 2; xb++){
                    Y_buff[(yy+yb)*x + (xx+xb)] *= Y;
                }
            }

            U_buff[(yy/2)*x/2 + xx/2] *= U;
            V_buff[(yy/2)*x/2 + xx/2] *= V;
        }
    }
